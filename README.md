...
*免安装的 AI 文字角色扮演游戏 —— 忠于士郎正宗原作漫画与 2026 新动画*
...
## ✦ 選局桌面 —— Windows 95 复古赛博终端

点「進入」，落地的不是一张开局列表，而是一整块 Windows 95 布局的复古赛博朋克电脑桌面：绿磷光单色像素图标、CRT 扫描线与滚动光带、抖动网纹窗口、点开文件夹的 8-bit 系统音效，背后仍是实时建城 3D 当壁纸。

![選局桌面](docs/media/desktop.png)

- **每个开局是一个文件夹**：十四个正典开局排成桌面文件夹，双击弹出 `C:\OPENINGS\…` 复古窗口——年代地点、GHOST 徽章、剧情简介、「進入」一应俱全，任务栏同步出现任务条
- **我的电脑 = 自定义开局**；桌面**右键**空白处还能「新建文件夹」并命名，点开即进开局向导（向导本身也套同款窗壳）
- **系统图标各司其职**：接入 AI（拨号上网）、和文回路（翻译设定）、世界书、存档、剧情简介.txt、開局碼（与友人共启同一局）、音乐室、**相册**（生图 + 图片缓存）、设置、音量
- **開始菜单 + 任务栏 + 实时时钟**：正宗 Win95 開始菜单侧边条与系统托盘，「关机」即回封面

<img src="docs/media/desktop_menu.png" alt="開始菜单" width="440"> <img src="docs/media/desktop_folder.png" alt="开局文件夹窗口" width="440">

### 你的东西，都是桌面上的文件

自己添加或导入的内容会以文件形式摊在桌面，随手管理，不必再钻进设置菜单：

- **世界书条目 / 自定义预设 / 自定义开局**，每一项都是一个可点开、可删除的桌面文件
- **拖进回收站即删除**（手机端在文件窗口内点「删除」）；系统项与十四个预设开局不可删，删除即时持久化
- **相册**一键直达生图接口设置与图片缓存管理

## ✦ 音乐室 —— 云唱片机 + 默认原声

音乐室内置默认原声《Utai IV: Reawakening》（川井憲次「謡」主题，Steve Aoki Remix），点开链接即随首次交互自动播放，面板附 LRC 歌词同步高亮。另有「云唱片机」可粘贴任意在线单曲 / 专辑 / 歌单链接云播放，不占本地空间。
